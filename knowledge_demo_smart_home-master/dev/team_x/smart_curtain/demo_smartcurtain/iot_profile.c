 ...
 *
 */
int IotProfile_Report(int curtainStatus)
{
    int ret = -1;
    cJSON *root;
    char *jsonString;
    IotProfileService    service;
    IotProfileKV  kvCurtainStatus;

    /* package the data */
    service.eventTime = NULL;
    service.serviceID = "SmartCurtain";
    service.propertyLst = &kvCurtainStatus;
    service.nxt = NULL;

    kvCurtainStatus.key = "CurtainStatus";
    kvCurtainStatus.value = curtainStatus ? "OPEN" : "CLOSE";
    kvCurtainStatus.type = IOT_PROFILE_KEY_DATATYPE_STRING;
    kvCurtainStatus.nxt = NULL;
 ...
 *
 */
static int DealSetCurtainStatus(cJSON *objCmd)
{
    int ret = -1;
    cJSON *objParas = NULL;
    cJSON *objPara = NULL;
    ...
    if ((objPara = cJSON_GetObjectItem(objParas, "CurtainStatus")) != NULL) {
        if (0 == strcmp(cJSON_GetStringValue(objPara), "OPEN")) {
            setCurtainParam.status = CN_BOARD_CURTAIN_OPEN;
        } else {
            setCurtainParam.status = CN_BOARD_CURTAIN_CLOSE;
        }

        ret = IotProfile_CommandCallback(CLOUD_COMMAND_SETCURTAIN_STATUS, &setCurtainParam);
    } else {
...
static int DealSetShedule(cJSON *objCmd)
{
    ...
    if ((objPara = cJSON_GetObjectItem(objParas, "Command")) != NULL) {
        cJSON *cmd;
        if ((cmd = cJSON_GetObjectItem(objPara, "CurtainStatus")) != NULL) {
            if (0 == strcmp(cJSON_GetStringValue(cmd), "OPEN")) {
                setSheduleParam.shedulecmd.cmd = CN_BOARD_CURTAIN_OPEN;
            } else {
                setSheduleParam.shedulecmd.cmd = CN_BOARD_CURTAIN_CLOSE;
            }
        }
    }
...
int CLOUD_CommandCallBack(const char *jsonString)
{
    ...
    if (0 == strcmp(cJSON_GetStringValue(objCmdName), "SetCurtainStatus")) {
        ret = DealSetCurtainStatus(objRoot);
