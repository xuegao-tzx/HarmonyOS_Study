 ...
 *    Reference Identifier：指示时钟参考源的标记，该字段只在服务器端有效。对于一级服务器，字段长度为4字节ASCII字符串，左对齐不足添零。对于二级服务器，在IPV4环境下，
 *                          取值为一级服务器的IP地址，在IPV6环境下，是一级服务器的NSAP地址。
...
int SntpGetRtcTime(int localTimeZone, struct tm *rtcTime)
{
    ...
    do {
        ...
        FD_SET(sockfd, &set);
        timeout.tv_sec = 0;
        timeout.tv_usec = 500000;   // 500000 = 500ms
            ...
            if (FD_ISSET(sockfd, &set)) {
                if (recvfrom(sockfd, (char *) &sntpPacket, sizeof(sntpPacket),
                 0, (struct sockaddr*) &server, &addrLen) < 0) {
                    ...
                    tv.tv_sec += (localTimeZone * 3600);    // 3600 = 1H
