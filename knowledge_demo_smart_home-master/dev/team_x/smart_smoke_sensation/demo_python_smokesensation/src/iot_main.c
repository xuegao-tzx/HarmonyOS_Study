...
#define SOFTAP_NAME    "teamX-Smoke01"
...
#define CONFIG_MQ2_THRESHOLD   1000                      // the threshold for the smoke sensor
...
static void SensorTaskEntry(const void *arg)
{
    ...
    while (1) {
        ...
        E53SF1_SetBeep(g_appController.beepStatus);
        (void) IotProfile_Report( g_appController.beepStatus, g_appController.mq2Value, g_appController.threshold);

        RaiseLog(LOG_LEVEL_INFO,"Get the sensor data:beep:%s mq2:%.2f threshold:%.2f",\
                g_appController.beepStatus?"ON":"OFF", g_appController.mq2Value, g_appController.threshold);
        osDelay(Time2Tick(CONFIG_SENSOR_SAMPLE_CYCLE));
    }
    return;
}
...
int IotProfile_CommandCallback(int command, void *buf)
{
    ...
    if (cmd == CLOUD_COMMAND_SETBEEP) {
        ...
        return E53SF1_SetBeep(g_appController.beepStatus);
...
int IotDeviceInit(void)
{
    E53SF1_Init();
    BOARD_InitWifi();
    g_appController.threshold = CONFIG_MQ2_THRESHOLD;
...
int ConnectHuaweiIoTPlatform(void)
{
    NfcInfo nfcInfo;     
    nfcInfo.deviceID = "6128bdd12cce4f02861e6d98_smartsmokesensation01";
