...
{
    NET_PRT("Network configure done.(result=%d)\n", result);
...
static int StaStart(void)
{
    ...
    if ((error != ERROR_WIFI_BUSY) && (error != WIFI_SUCCESS)) {
        NET_ERR("EnableWifi failed, error = %d\n", error);
        return -1;
    }
    ...
    if (error != WIFI_SUCCESS) {
        NET_ERR("RegisterWifiEvent failed, error = %d\n", error);
        return -1;
    }
...
static int WapStaConnect(WifiDeviceConfig *config)
{
    ...
    if (error != WIFI_SUCCESS) {
        NET_ERR("AddDeviceConfig add config failed, error=%d\n", error);
        return -1;
    }
    int count = 0;
    while (count < FAST_CONNECT_RETRY_NUM) {
        ...
        NET_PRT("[sample]continue\n");
        count++;
    }

    if (error != WIFI_SUCCESS) {
        NET_ERR("ConnectTo conn failed %d\n", error);
        return -1;
    }

    NET_PRT("WapSta connecting...\n");
    return 0;
}
...
{
    NET_PRT("ParseWifiData vendorData len:%d\n", len);
    if (wifiInfo == NULL) {
...
int SendRawEncodeData(const unsigned char *data, size_t len)
{
    ...
    if (ret != 0) {
        NET_ERR("SendRawEncodeData base64 encode fial\n");
        return -1;
    }
    size_t encodeBufLen = writeLen;
    if (writeLen > MAX_DATA_LEN) {
        NET_ERR("SendRawEncodeData dataLen overSize\n");
        return -1;
    }
    char *buf = malloc(writeLen + 1);
    if (buf == NULL) {
        NET_ERR("Malloc failed\r\n");
        return -1;
    }
    ...
    if (mbedtls_base64_encode((unsigned char *)buf, encodeBufLen, &writeLen, (const unsigned char *)data, len) != 0) {
        NET_ERR("SendRawEncodeData base64 encode failed\r\n");
        free(buf);
        buf = NULL;
        return -1;
    }
