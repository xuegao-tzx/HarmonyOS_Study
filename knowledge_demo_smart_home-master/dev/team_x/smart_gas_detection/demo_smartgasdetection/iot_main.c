...
static void SensorTaskEntry(const void *arg)
{
    ...
    while (1) {
        ...
        ret = E53SF1_GetMq2(&g_appController.mq5Value);
        if (ret != 0) {
            RaiseLog(LOG_LEVEL_ERR, "Get the sensor data error");
            osDelay(Time2Tick(CONFIG_SENSOR_SAMPLE_CYCLE));
            continue;
        }
        if (g_appController.mq5Value > g_appController.threshold) {
        ...
        (void) IotProfile_Report(g_appController.beepStatus, g_appController.mq5Value, g_appController.threshold);
         RaiseLog(LOG_LEVEL_INFO,"Get the sensor data:beep:%s mq2:%.2f threshold:%.2f",\
            g_appController.beepStatus ? "ON" : "OFF", g_appController.mq5Value, g_appController.threshold);
        osDelay(Time2Tick(CONFIG_SENSOR_SAMPLE_CYCLE));
    }
    return;
}
...
#define    SOFTAP_NAME    "teamX-GasDet01"
...
static void IotMainTaskEntry(const void *arg)
{
    ...
    nfcInfo.deviceID = "61445c41f74139027d2233e5_GasDet01";
