...
int ParseNetCfgData(const struct WifiInfo *wifiInfo, const unsigned char *vendorData, unsigned int len)
{
    ...
    if (memcpy_s(g_netCfg.ssid, sizeof(g_netCfg.ssid), wifiInfo->ssid, wifiInfo->ssidLen) != EOK) {
        RaiseLog(LOG_LEVEL_ERR, "Copy ssid failed\n");
        return -1;
    }
    if (memcpy_s(g_netCfg.preSharedKey, sizeof(g_netCfg.preSharedKey), wifiInfo->pwd, wifiInfo->pwdLen) != EOK) {
        RaiseLog(LOG_LEVEL_ERR, "Copy pwd failed\n");
        return -1;
    } 
...
{
    if (memcpy_s(g_ssid, sizeof(g_ssid), apName, strlen(apName)) != EOK) {
        RaiseLog(LOG_LEVEL_ERR, "copy apName failed\n");
        return -1;
    }

    NetCfgRegister();
  
    while (strlen(g_netCfg.ssid) == 0) {
        osDelay(100);
    }
    
    RaiseLog(LOG_LEVEL_DEBUG, "g_netCfg.ssid :%s g_netCfg.preSharedKey :%s\n", g_netCfg.ssid, g_netCfg.preSharedKey);
    if (memcpy_s(ssid, sLen, g_netCfg.ssid, sizeof(g_netCfg.ssid)) != EOK) {
        RaiseLog(LOG_LEVEL_ERR, "copy ssid failed\n");
        return -1;
    }

    if (memcpy_s(pwd, pLen, g_netCfg.preSharedKey, sizeof(g_netCfg.preSharedKey)) != EOK) {
        RaiseLog(LOG_LEVEL_ERR, "copy pwd failed\n");
        return -1;
    }
    RaiseLog(LOG_LEVEL_DEBUG, "ssid :%s pwd :%s\n", ssid, pwd);
    return 0;
}